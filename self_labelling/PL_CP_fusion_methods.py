    ...
    for vid in features_dict.keys():
        ...
        for ix in range(len(labels)):
            ...
            if length > args.local_fusion_th_min and length < args.local_fusion_th_max:
                algo = rpt.Pelt(
                    model=args.local_fusion_model, jump=args.local_fusion_jump
                ).fit(features[start:end])
    ...
    for vid in tqdm(features_dict.keys()):
        ...
        for pen in args.CMSL_pen:
            ...
            if os.path.exists(
                ...
                algo = rpt.Pelt(model=args.CMSL_model, jump=args.CMSL_jump).fit(
                    features
                )
                res = algo.predict(pen=pen)
        ...
        if args.CMSL_th_refine < 9000:
            ...
            for cp_pos in CP_starts:
                ...
                for ix, (PL_start, PL_end) in enumerate(zip(PL_starts, PL_ends)):
                    if (
                        abs(PL_start - cp_pos) < args.CMSL_th_refine
                        and abs(PL_start - cp_pos) < dist
                    ):
                        refine = True
                        dist = abs(PL_start - cp_pos)
                        PL_ix = ix
                    if (
                        abs(cp_pos - PL_end) < args.CMSL_th_refine
                        and abs(cp_pos - PL_end) < dist
                    ):
                        refine = True
