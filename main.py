...
from self_labelling.PL_CP_fusion_methods import (
    get_save_local_fusion,
    merge_PL_CP,
    CMPL,
    extract_CP,
)
...
def main(args, device, model_load_dir, model_save_dir, results_save_dir):
    ...
    if args.action == "train" and args.extract_save_pseudo_labels == 0:
        ...
        test_loader = DataLoader(
            args, args.test_data, args.extract_set, results_dir=results_save_dir
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_data", default="bslcp", choices=["bslcp", "phoenix14"])

    parser.add_argument("--test_data", default="bslcp", choices=["bslcp", "phoenix14"])

    parser.add_argument("--i3d_training", default="i3d_kinetics_bslcp_981")

    parser.add_argument("--num_in_frames", default=16, type=int)
    parser.add_argument(
        "--folder", default="", type=str, help="folder to save the results"
    )
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="use only a subset of the test set for evaluation and visualization",
    )
    parser.add_argument(
        "--viz_results",
        action="store_true",
        help="save visualizations of results and gt for each video sequence",
    )

    ### target_finetuning settings
    parser.add_argument(
        "--extract_save_pseudo_labels",
        default=0,
        type=int,
        choices=[0, 1],
        help="extract and save pseudo_labels of type pseudo_label_type",
    )
    parser.add_argument(
        "--pseudo_label_type",
        default="CP",
        choices=["CMPL", "merge", "local", "PL", "CP"],
        type=str,
    )
    ...
    parser.add_argument(
        "--use_test",
        action="store_true",
        help="append test set to extract_set (for combined train and test set",
    )

    parser.add_argument(
        "--pretrained", default="", type=str, help="Path to pretrained model"
    )

    parser.add_argument(
        "--use_pseudo_labels",
        action="store_true",
        help="Use already extracted pseudo-labels for training ",
    )
    parser.add_argument(
        "--load_label_path", default="", type=str, help="Path to pseudo-labels"
    )
    ...
    parser.add_argument(
        "--eval_use_CP",
        action="store_true",
        help="CP Baseline, use changepoints as predictions (without training)",
    )
