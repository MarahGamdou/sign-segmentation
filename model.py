    ...
    def __init__(
        ...
        if weights is None:
            ...
            self.ce = nn.CrossEntropyLoss(
                weight=torch.tensor(weights).to(device), ignore_index=-100
            )
    ...
    def train(
        ...
        for epoch in range(num_epochs):
            ...
            while batch_gen.has_next():
                ...
                batch_input, batch_target, batch_target_eval, mask = (
                    batch_input.to(device),
                    batch_target.to(device),
                    batch_target_eval.to(device),
                    mask.to(device),
                )
                optimizer.zero_grad()
                ...
                bar.suffix = "({batch}/{size}) Batch: {bt:.1f}s | Total: {total:} | ETA: {eta:} | Loss: {loss:}".format(
                    ...
                    eta=datetime.timedelta(
                        seconds=ceil((bar.eta_td / batch_size).total_seconds())
                    ),
                    loss=loss.item(),
                )
                count += 1
            ...
            result_dict = get_metrics_train.save_print_metrics(
                self.writer,
                save_dir,
                epoch,
                epoch_loss / (len(batch_gen.list_of_examples) / batch_size),
            )
    ...
    def predict(
        ...
        with torch.no_grad():
            ...
            for vid in tqdm(vid_list_file):
                ...
                if CP_dict is not None:
                    ...
                    if self.num_classes == 1:
                        ...
                        predicted = torch.tensor(
                            np.where(
                                np.asarray(pred_prob) > args.classification_threshold,
                                1,
                                0,
                            )
                        ).to(device)
                    ...
                    if save_pslabels:
                        ...
                        save_score_dict[vid]["preds"] = np.asarray(
                ...
                get_metrics_test.calc_scores_per_batch(
                    predicted.unsqueeze(0), gt.unsqueeze(0), gt_eval.unsqueeze(0)
                )
            ...
            if save_pslabels:
                ...
                for episode in PL_labels_dict.keys():
                    PL_root = (
                        ...
                        .replace(f"exps/results/regression", "data/pseudo_labels/PL")
                        .replace(
                            f"exps/results/classification", f"data/pseudo_labels/PL"
                        )
                    )
                    ...
                    if not os.path.exists(f"{PL_root}/{episode}"):
                        ...
                        pickle.dump(
                            PL_labels_dict[episode],
                            open(f"{PL_root}/{episode}/preds.pkl", "wb"),
                        )
                        pickle.dump(
                            PL_scores_dict[episode],
                            open(f"{PL_root}/{episode}/scores.pkl", "wb"),
                        )
                    else:
            ...
            result_dict = get_metrics_test.save_print_metrics(
                self.writer, save_dir, epoch, epoch_loss / len(vid_list_file)
            )
