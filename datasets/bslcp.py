...
class bslcp:
    ...
    def get_features(self, args):
        ...
        gt_list = [
            glosses
            for ix, glosses in enumerate(
                self.info_data["videos"]["alignments"]["boundaries"]
            )
            if self.info_data["videos"]["split"][ix] == self.setvalue
        ]
        ...
        if args.feature_normalization:
            features = np.nan_to_num(
                (features - np.mean(features, axis=0)) / np.std(features)
            )
        ...
        self.vid_list = [
            vid for ix, vid in enumerate(self.vid_list) if ix not in list(del_list[0])
        ]
        ...
        if args.eval_use_CP:
            self.CP_dict = get_CP_dict(
                self.test_features_dict, self.test_vid_list, self.test_gt_dict
            )
