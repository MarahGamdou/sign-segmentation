...
class phoenix14:
    def __init__(
        ...
        self.vid_list = [
            video_name
            for ix, video_name in enumerate(self.info_data["videos"]["name"])
            if self.info_data["videos"]["split"][ix] == self.setvalue
        ]
        # for training with self labels
        self.test_vid_list = [
            video_name
            for ix, video_name in enumerate(self.info_data["videos"]["name"])
            if self.info_data["videos"]["split"][ix] == 2
        ]
    ...
    def get_features(self, args):
        ...
        gt_list = [
            glosses
            for ix, glosses in enumerate(boundaries)
            if self.info_data["videos"]["split"][ix] == self.setvalue
        ]
        gt_list_eval = [
            glosses
            for ix, glosses in enumerate(boundaries_eval)
            if self.info_data["videos"]["split"][ix] == self.setvalue
        ]
        ...
        if args.regression:
            ...
            if self.setname == "train":
                        ...
                        if label == 1:
                            ...
                            if label_old:
                                gauss.append(
                                    gauss_dist(
                                        len(gt),
                                        np.mean(np.asarray(boundary_frames)),
                                        std,
                                    )
                                )
                            boundary_frames = []
                ...
                gt_list_eval = dilate_boundaries(
                    [[int(round(item)) for item in sublist] for sublist in gt_list]
                )
        ...
        num_frames_list = [
            max(t - (args.num_in_frames - 1), 0)
            for ix, t in enumerate(self.info_data["videos"]["videos"]["T"])
            if self.info_data["videos"]["split"][ix] == self.setvalue
        ]
        ...
        for ix, video in enumerate(self.vid_list):
            if len(gt_list[ix]) == 0:
                ...
                continue
            self.gt_dict[video] = gt_list[ix][
                int(args.num_in_frames / 2) : -int(args.num_in_frames / 2) + 1
            ]
        ...
        for ix, video in enumerate(self.vid_list):
            if len(gt_list[ix]) == 0:
                del_videos.append(video)
                continue
            self.eval_gt_dict[video] = gt_list_eval[ix][
                int(args.num_in_frames / 2) : -int(args.num_in_frames / 2) + 1
            ]
        ...
        self.vid_list = [
            vid for ix, vid in enumerate(self.vid_list) if vid not in del_videos
        ]
        ...
        if (
            args.use_pseudo_labels
            or (
                args.extract_save_pseudo_labels
                and args.pseudo_label_type != "PL"
                and args.pseudo_label_type != "CP"
            )
        ) and self.setname == "train":
            self.gt_dict = {}
            ...
            if args.use_test:
                # load features
                test_features_data = loadmat(
                    Path(f"{self.test_features_path}/features.mat")
                )
                test_features = features_data["preds"]
                test_num_frames_list = [
                    max(t - (args.num_in_frames - 1), 0)
                    for ix, t in enumerate(self.info_data["videos"]["videos"]["T"])
                    if self.info_data["videos"]["split"][ix] == 2
                ]
