    ...
    for i, sen in enumerate(glosses):
        ...
        for ix, gloss in enumerate(sen):
            if gloss != old_gloss:
                boundaries[-1] = [
                    1
                    if i >= ix - round(num_boundary_frames / 2)
                    and i < ix + max(0, num_boundary_frames // 2)
                    else old
                    for i, old in enumerate(boundaries[-1])
                ]
            old_gloss = gloss
        ...
        if uniform:
            signs.append(
                (sum(boundaries[i][8:-7]) // num_boundary_frames) + 1
            )  # hard coded in_frames 16 need to change
    ...
    for i, sen in enumerate(glosses):
        ...
        for ix, gloss in enumerate(sen):
            if gloss != old_gloss:
                counter += 1
                start = ix - min(
                    num_boundary_frames // 2, round(len_instances[counter - 1] / 4)
                )
                end = ix + min(
                    num_boundary_frames // 2, round(len_instances[counter] / 4)
                )
